...
void GPIOCallback(lv_timer_t * timer) {
  ...
  // --- AI1 ---
  snprintf(buf, sizeof(buf), "%.1f V", voltage0);
  lv_label_set_text(ui_AI1, buf);


  // --- AI2 ---
  snprintf(buf, sizeof(buf), "%.1f V", voltage3);
  lv_label_set_text(ui_AI2, buf);

  // --- AI3 ---
  snprintf(buf, sizeof(buf), "%.1f V", voltage1);
  lv_label_set_text(ui_AI3, buf);


  // --- AI4 ---
  snprintf(buf, sizeof(buf), "%.1f V", voltage2);
  lv_label_set_text(ui_AI4, buf);

  lv_arc_set_value(ui_Arc1, voltage0); // current0 = 0–20 mA
  lv_arc_set_value(ui_Arc2, voltage3);
  lv_arc_set_value(ui_Arc3, voltage1);
  lv_arc_set_value(ui_Arc4, voltage2);
...
void setTransistorState(uint8_t output, bool state) {
  switch (output) {
    case 1: io.digitalWrite(OUTPUT1, state ? HIGH : LOW); break;
    case 2: io.digitalWrite(OUTPUT2, state ? HIGH : LOW); break;
    case 3: io.digitalWrite(OUTPUT3, state ? HIGH : LOW); break;
    case 4: io.digitalWrite(OUTPUT4, state ? HIGH : LOW); break;
  }


  // Serial.printf("TR%d -> %s\n", output, state ? "ON" : "OFF");
